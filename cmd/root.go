...
func init() {
	...
	{
		sshCmd.PersistentFlags().StringVarP(&User, "user", "u", "root", "用户名, 例如 root")
		sshCmd.PersistentFlags().StringVarP(&Password, "password", "P", "123456", "密码, 例如 123456")
		sshCmd.Flags().BoolVarP(&NetworkFailed, "network", "n", false, "是否显示因为网络错误而失败的IP")
		sshCmd.Flags().BoolVarP(&AuthenticationFailed, "auth", "a", false, "是否显示因为认证错误而失败的IP")
