# SSH Scanner

一个高性能的局域网 SSH 扫描工具，支持并发扫描和密码/密钥认证。

## 使用场景

我在我的局域网里有50多台设备，我记得我的设备的密码都是一样的，但是有一个例外，我怎么找到他？
这个程序就是在这种情景下出现的。借由 goroutine，这个工作可以轻松的并发完成。

## 功能特性

- ✨ **交互式 UI**：使用 [bubbletea](https://github.com/charmbracelet/bubbletea) 提供美观的终端界面
- 🔄 **实时扫描动画**：扫描时显示旋转动画和进度
- 📊 **动态结果显示**：实时显示扫描结果，无需等待扫描完成
- ⚡ **高并发扫描**：默认 500 个并发 worker，快速完成扫描
- 🎯 **智能退出**：按 `q` 键或 `Ctrl+C` 优雅退出
- 🔐 **多种认证方式**：支持密码和公钥认证
- 📤 **多种输出格式**：支持 console 和 JSON 格式

## 安装

```bash
go build -o scanner .
```

## 使用方法

### 基本用法

```bash
# 使用默认参数扫描（交互式 UI）
./scanner ssh

# 指定网段和 IP 范围
./scanner ssh -p 3 -s 1 -e 254

# 指定用户名和密码
./scanner ssh -u root -P mypassword
```

### 高级选项

```bash
# 显示认证失败的主机
./scanner ssh -a

# 显示网络错误的主机
./scanner ssh -n

# 显示所有详细信息（使用传统输出，不使用 bubbletea UI）
./scanner ssh -v

# 启用公钥登录
./scanner ssh --pubkey

# 循环检索模式
./scanner ssh -l

# 指定自定义 SSH 端口
./scanner ssh --port 2222

# JSON 格式输出
./scanner ssh --output-format json
```

### 参数说明

#### 全局参数

- `-p, --prefix`：网段，例如 3 表示 192.168.3.x（默认：3）
- `-s, --start`：起始 IP 的最后一位（默认：1）
- `-e, --end`：结束 IP 的最后一位（默认：254）
- `--output-format`：输出格式，可选 console 或 json（默认：console）
- `-v, --verbose`：显示详细信息（禁用 bubbletea UI）

#### SSH 命令参数

- `-u, --user`：SSH 用户名（默认：root）
- `-P, --password`：SSH 密码（默认：123456）
- `--port`：SSH 端口（默认：22）
- `-a, --auth`：显示认证失败的 IP
- `-n, --network`：显示网络错误的 IP
- `--pubkey`：启用公钥登录
- `-l, --loop`：循环检索模式

## 交互式 UI 说明

默认情况下，SSH 扫描使用 bubbletea 提供的交互式界面：

- **扫描动画**：显示旋转的扫描指示器和进度（已扫描/总数）
- **实时结果**：
  - ✓ 绿色显示成功登录的主机
  - ⚠ 黄色显示认证失败的主机（需要 `-a` 参数）
  - ✗ 红色显示网络错误的主机（需要 `-n` 参数）
- **退出方式**：按 `q` 键或 `Ctrl+C` 退出扫描

## 示例

### 扫描局域网中的 SSH 服务

```bash
# 扫描 192.168.3.1 到 192.168.3.254，使用默认端口 22
./scanner ssh -p 3 -s 1 -e 254 -u root -P 123456

# 扫描自定义端口的 SSH 服务
./scanner ssh -p 3 -s 1 -e 254 -u root -P 123456 --port 2222
```

### 查看所有失败的连接

```bash
# 显示认证失败和网络错误的主机
./scanner ssh -a -n
```

### 获取 JSON 格式结果

```bash
# 输出 JSON 格式，方便脚本处理
./scanner ssh --output-format json > result.json
```

## 性能

- 默认并发数：500 个 worker
- 连接超时：500ms
- 适合快速扫描大量 IP

## 技术栈

- [Cobra](https://github.com/spf13/cobra)：命令行框架
- [Bubbletea](https://github.com/charmbracelet/bubbletea)：终端 UI 框架
- [Bubbles](https://github.com/charmbracelet/bubbles)：Bubbletea 组件库
- [Lipgloss](https://github.com/charmbracelet/lipgloss)：终端样式库
- [golang.org/x/crypto/ssh](https://pkg.go.dev/golang.org/x/crypto/ssh)：SSH 客户端

## 其他命令

输入 `-h` 查看完整帮助信息：

```bash
./scanner -h
./scanner ssh -h
```
